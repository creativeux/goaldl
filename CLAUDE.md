...
- Live BLM view: `monitor -blm` streams frames through `pkg/stream`'s `BLMView`, which drives the same `pkg/blm.Grid`, redraws a compact heatmap in place on a TTY (active cell reverse-highlighted, closed/open-loop status), and prints the final Average + Correction tables on exit.
