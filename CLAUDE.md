...
- `cmd/goaldl/tui.go` - **Dashboard** (Bubble Tea), the default UX: seven tabs (keys 1-7) — sensor table (dual-unit ALT column + always-on MIN/MAX), BLM / INT / O2 fuel-trim grids, flag data (MW2/MWAF1/MCU2IO checklists), error codes (MALFFLG1-3), and a WinALDL-style scrolling raw-byte history — driven by a `stream.Session` (live `-p` or replay). A **persistent loop-state line** (green CLOSED / amber OPEN + per-grid ●/○ recording dots) sits under the tab bar on every tab, since loop state governs whether the BLM/INT grids are accumulating; `s` saves all three grids to timestamped files, `c` clears the active grid (or resets sensor MIN/MAX). The model runs the session in a goroutine and receives `Snapshot`s over a channel; decoded views render from the last ParseOK snapshot (a bad sample only updates the raw view and the red/green footer heartbeat — WinALDL-style gating, a consumer-level decision per the data policy). INT/O2 grids and extrema are **consumer-side accumulation** over the same `Snapshot` stream (no Snapshot/Session/blm/ecm change). View rendering reuses the pure builders in `pkg/stream` (`SensorTable`/`SensorTableExtrema`, `BLMBody`/`INTBody`/`O2Body` over a shared `gridHeat`, `LoopStatus`, `FlagsBody`, `CodesBody`, `RawHistory`). Model logic is unit-tested in `tui_test.go` (incl. a full-fixture end-to-end run). WinALDL-parity roadmap: `specs/2026-07-04_feature_winaldl-parity/` — Phase 1 (diagnose) shipped 2026-07-04; Phase 2 (tune: INT/O2 grids, in-TUI save/clear, sensor Min/Max, loop-state chrome) shipped + verified 2026-07-04; next: Phase 3 (recording toggle, replay pause/speed keys, spark grid) and a `serve` adapter proving the `Session` API drives a non-terminal front-end.
