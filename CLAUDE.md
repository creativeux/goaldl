...
- `cmd/goaldl/tui.go` - **Dashboard** (Bubble Tea), the default UX: five tabs — sensor table (dual-unit ALT column), BLM grid, flag data (MW2/MWAF1/MCU2IO checklists), error codes (MALFFLG1-3), and a WinALDL-style scrolling raw-byte history — driven by a `stream.Session` (live `-p` or replay). The model runs the session in a goroutine and receives `Snapshot`s over a channel; decoded views render from the last ParseOK snapshot (a bad sample only updates the raw view and the red/green footer heartbeat — WinALDL-style gating, a consumer-level decision per the data policy). View rendering reuses the pure builders in `pkg/stream` (`SensorTable`, `BLMBody`, `FlagsBody`, `CodesBody`, `RawHistory`). Model logic is unit-tested in `tui_test.go`. WinALDL-parity roadmap: `specs/2026-07-04_feature_winaldl-parity/plan.md` — Phase 1 (diagnose parity) shipped 2026-07-04; next: Phase 2 (INT/O2 grids, in-TUI save/clear, sensor Max/Min), Phase 3 (recording toggle, replay pause/speed keys, spark grid), and a `serve` adapter proving the `Session` API drives a non-terminal front-end.
