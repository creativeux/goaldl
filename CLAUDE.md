...
- `go run ./cmd/goaldl -tcp 192.168.4.1:3333` - Dashboard, live from a TCP ALDL bridge (ESP32 forwarding the raw UART byte stream over WiFi/Ethernet); `-tcp` also works on `monitor`. Sources are mutually exclusive: one of `-p` / `-tcp` / capture file.
...
- `go run ./cmd/goaldl blm session.raw -xdf 42.xdf -table "main ve" -paste ve.txt` - Same, but on the named XDF table's own axes + a TunerPro paste-with-multiply block (`-xdf` alone lists the definition's tables; both legacy-text and XML XDFs parse via `pkg/xdf`)
...
- `pkg/stream/` - `Session`/`Snapshot` (core API) + Provider abstraction: `ReplayProvider` (capture file), `SerialProvider` (live ECM, optional raw recording), and `TCPProvider` (live over a TCP byte pipe — an ESP32 bridge forwarding the UART stream; same Sink tee, same never-give-up reconnect contract, redials a fixed `Addr` with dial/read deadlines + a cancel-closer goroutine) emit `FrameEvent`s. Pure content builders `SensorTable`/`BLMBody`/`FlagsBody`/`CodesBody`/`RawHistory` produce terminal-view strings shared by the `monitor` renderers and the `tui`. `Snapshot` carries `Sensors` + `FuelTrim` + `Flags` + `Codes` + PROM/parse quality flags — all plain serializable data. Providers, `Session`, `BuildRows`, the view builders, and pacing are unit-tested against the drive fixture and the ground-truth log.
...
- `pkg/xdf/` - read-only TunerPro XDF parser (both the legacy text format and the v5+ XML format, sniffed): table titles, dimensions, X/Y axis labels/units — just enough for the `blm -xdf` correction export. Deliberately **no** Z data, addresses, or bin math (bin editing is TunerPro's job; see MISSION.md Positioning). Imports no other goaldl package; the future ADX importer's sibling. Ground truth: real $42 definitions in `data/xdf/` (gitignored pending license review; from-scratch fixtures in `pkg/xdf/testdata/` mirror their structure, and a skip-guarded `TestRealXDF` exercises the real files when present).
