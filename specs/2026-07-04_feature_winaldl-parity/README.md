...
### Post-implementation tweak 2 (user feedback, on PR #4)
- 2026-07-04: **Grid explainers** — user: "missing is an explanation of what each table MEANS, specifically BLM / INT / O2 / Spark". User chose the always-visible form (over a `?` help toggle): each grid tab now renders a dim 4–6 line "what this table means" block in place of the one-line legend — what the table is, how to read it, how to act on it (BLM: multiply base VE/fuel by avg/128; INT: sustained averages, cross-check BLM; O2: ~0.45 V stoich, stuck cells = masked offset; Spark: goal 0, repetition = pull timing/add fuel). Explainer constants live beside the builders in `pkg/stream/gridviews.go`; `monitor -blm` keeps the compact legend via unchanged `BLMBody` (new `BLMBodyExplained` dashboard variant, shared `blmBody` core). New `TestGridExplainers` + per-tab TUI assertions; spark never-dim assertion scoped to the grid rows. Full `-race` suite green; spec §8b added. Rendered output visually checked (explainer ≤78 chars/line).
