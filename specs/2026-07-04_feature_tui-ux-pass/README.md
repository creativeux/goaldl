...
- 2026-07-04: **Verify (self):** `go test -race ./...` green; `go vet` + `gofmt` clean; decoder goldens byte-identical; `blm` 469; layering seam (`session.go`/`stream.go`/`ecm`/`decoder`/`blm`) diff empty. New tests: `TestGridWidthTruncation`, `TestSensorTableColumnDrop`, `TestTUIWidthFit`. **Deferred**: sub-6-row degenerate height (evaluator note #2) — low value. Phase B is now complete (B.1+B.2+B.4; B.3 reverted). Awaiting a fresh-context `verify-feature` pass on B.2.
