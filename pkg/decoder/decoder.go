...
func (d *Decoder) Decode(data []byte) []Frame {
	...
	for _, b := range data {
		if f := d.Feed(b); f != nil {
			// Drop implausible frames: reject any frame whose RPM byte reads
			// an unrealistically high value (likely noise) so downstream
			// consumers only ever see clean data.
