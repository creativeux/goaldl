...
- [x] **Phase B — Layout resilience** ✅ B.1 + B.4 shipped + verified (2026-07-04): pinned tab bar/loop line/footer with a height-clamped scrollable body (`j`/`k`/↑/↓) so a short terminal never scrolls the tabs off the top (F4); grid explainers collapsed behind an `i` info accordion (compact legend by default). B.3 (collapse empty rows) was implemented then reverted at user request — the full grid always renders every row. **Remaining**: B.2 width awareness (narrow-terminal wrapping)
- [ ] **Phase C — Session safety**: dirty-grid tracking, quit confirm with recording/unsaved data, clear undo/confirm, exit summary
