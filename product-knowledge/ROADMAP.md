...
- [ ] **Phase C — Session safety + unified outputs** *(spec'd 2026-07-05)*: WinALDL-style output checklist collapsing `s`/`r`/`d` into **Save Buffer** (retroactive, bounded decoded-frame ring, no RAW) + **Log** (forward, crash-tolerant, RAW optional); dirty-grid tracking, quit confirm, clear undo (`u`), exit summary. Consumer-side only
