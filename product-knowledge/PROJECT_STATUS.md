...
-   [ ] **Epic: TUI UX pass** (spec: `specs/2026-07-04_feature_tui-ux-pass/`) — heuristic evaluation done 2026-07-04 (19 findings F1–F19, ranked S1–S4); five-phase plan drafted, recommended cut A→B→C now
    ...
    -   [~] Feature: Phase B layout resilience — **B.1+B.4 verified, B.2 implemented 2026-07-04** ([spec-phaseB.md](../../specs/2026-07-04_feature_tui-ux-pass/spec-phaseB.md)): B.1 pinned chrome + height-clamped scrollable body; B.4 explainer accordion (`i`); **B.2 width awareness** — grids truncate MAP columns at a whole-column boundary with a `›` cue, sensor table drops ALT→RAW, and `fitWidth` (ANSI-aware via `x/ansi`) truncates every frame line so chrome never soft-wraps (closes the A+B evaluator's note #1). **B.3 reverted** ("show the full table always"). B.2 self-verified (`-race` green, goldens byte-identical, `blm` 469, layering seam empty; `x/ansi` promoted indirect→direct, `go.sum` unchanged) — **awaiting fresh-context verify-feature**. Phase B feature-complete bar the sub-6-row degenerate-height note.
