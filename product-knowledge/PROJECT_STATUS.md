...
-   [ ] **Epic: TUI UX pass** (spec: `specs/2026-07-04_feature_tui-ux-pass/`) — heuristic evaluation done 2026-07-04 (19 findings F1–F19, ranked S1–S4); five-phase plan drafted, recommended cut A→B→C now
    ...
    -   [~] Feature: Phase B layout resilience — **B.1+B.4 shipped + verified 2026-07-04** ([spec-phaseB.md](../../specs/2026-07-04_feature_tui-ux-pass/spec-phaseB.md), [evaluation.md](../../specs/2026-07-04_feature_tui-ux-pass/evaluation.md) PASS): B.1 pinned chrome + height-clamped scrollable body (`View` guarantees frame ≤ terminal height, `j`/`k`/↑/↓ scroll a tall body so the tab bar never scrolls off); B.4 explainer accordion (compact legend by default, `i` toggles the full explainer). **B.3 (collapse empty grid rows) implemented then reverted at user request** — "show the full table all the time". Corrected F4: the standard alt-screen renderer scrolls the *top* off, not the bottom. **Remaining**: B.2 width awareness (spark/sensor tables wrap when narrow — evaluator's two out-of-scope notes both fall here).
